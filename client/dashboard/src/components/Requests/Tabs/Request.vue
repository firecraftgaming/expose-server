...
import {bodyIsJson, copyToClipboard, isEmptyObject, isNestedStructure, toJson} from '@/lib/utils'
    ...
    <div class="max-w-full px-6 pt-3">
            ...
            <AccordionItem value="pluginOpen">
                <AccordionTrigger>
                    ...
                    <template v-slot:action>
                        <IconCopyButton @click="copyToClipboard(toJson(request.plugin.details))">
                            Copy as JSON
                        </IconCopyButton>
                    </template>
        ...
        <div v-if="Object.keys(request.query).length > 0"
            ...
            <div class="flex flex-1 items-center justify-between mb-3 pl-4 pr-2 font-medium">
                ...
                <div>
                    <IconCopyButton @click="copyToClipboard(toJson(request.query))">
                        Copy as JSON
                    </IconCopyButton>
                </div>
            </div>
        ...
        <Accordion type="single" collapsible v-model="postParametersAccordionState"
            ...
            <AccordionItem value="postParametersOpen">
                <AccordionTrigger>
                    ...
                    <template v-slot:action>
                        <IconCopyButton @click="copyToClipboard(toJson(request.post))">
                            Copy as JSON
                        </IconCopyButton>
                    </template>
                    <template v-slot:icon>
        ...
        <Accordion type="single" collapsible v-model="accordionState">
            <AccordionItem value="requestHeaderOpen">
                <AccordionTrigger>
                    <div>Headers</div>
                    <template v-slot:action>
                        <IconCopyButton @click="copyToClipboard(toJson(request.headers))">
                            Copy as JSON
                        </IconCopyButton>
                    </template>
                    <template v-slot:icon>
